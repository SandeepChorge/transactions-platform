...
| [#16 — Dashboard & Insights (V3)](https://github.com/SandeepChorge/transactions-platform/issues/16) | Dashboard, search, budgets, widgets, Phase 6 → Phase 12 | Open — **Phase 6 done (#29–#31, #33), Phase 7 done (#34)**. Next up is Phase 8, the category insight carousel |
...
- **Nothing decides for itself which dashboards Home shows.** `DashboardLayout` in `feature/dashboard/domain` answers "which dashboards, in what order, and which one opens first?" as pure derivations, and the manage screen, the default picker and Home all read it. Filtering or sorting the catalog at a call site is how three screens end up disagreeing — two rules in particular are easy to re-derive wrongly: an empty enabled set falls back to the *whole* catalog rather than showing nothing, and a starred dashboard stops counting the moment it is switched off.
- **Preferences store the *disabled* set, not the enabled one.** They look interchangeable and are not. Stored-as-enabled means a dashboard added in a later release is switched off for every existing account and never discovered; stored-as-disabled means it arrives switched on and appended. The same reasoning applies to any future on/off catalog.
- **A stored dashboard id is `DashboardKey.storageId` — `"PULSE"` or `"custom:<uuid>"`.** Never an enum ordinal, and never the `DashboardId` enum itself, which a user-built dashboard has no entry in. Unknown ids are dropped on read, which is deliberate: it is what lets a deleted custom dashboard leave the stored order alone, and what will absorb a dashboard removed in a future release.
